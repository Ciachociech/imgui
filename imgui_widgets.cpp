...
{
    if (ImGuiWindow* window = GetCurrentWindow(); window->SkipItems)
        return;
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (g.NavActivateDownId == id)
    {
        ...
        if ((g.NavActivateId == id) || nav_activated_by_inputs)
        {
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    if ((window->OuterRectClipped.GetArea() / bb.GetArea()) < 1.5f)
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_visible_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    if (held && alpha >= 1.0f && grab_h_norm < 1.0f)
    {
...
void ImGui::ShrinkWidths(ImGuiShrinkWidthItem* items, int count, float width_excess)
{
    ...
    while (width_excess > 0.0f)
        for (auto n = 0; n < count && width_excess > 0.0f; n++)
        {
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float preview_width = ((flags & ImGuiComboFlags_WidthFitPreview) && (preview_value)) ? CalcTextSize(preview_value, NULL, true).x : 0.0f;
    ...
    if (ButtonBehavior(bb, id, &hovered, &held) && !popup_open)
    {
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (g.ActiveIdIsJustActivated || is_already_past_limits_and_pushing_outward)
    {
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (temp_input_is_active)
    {
        ...
        const bool is_clamp_input = (flags & ImGuiSliderFlags_AlwaysClamp) != 0 && (!p_min || !p_max || DataTypeCompare(data_type, p_min, p_max) < 0);
...
bool ImGui::DragScalarN(const char* label, ImGuiDataType data_type, void* p_data, int components, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (const char* label_end = FindRenderedTextEnd(label); label != label_end)
    {
        SameLine(0, g.Style.ItemInnerSpacing.x);
...
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min, TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    {
        // Linear slider
        if (const bool is_floating_point = (data_type == ImGuiDataType_Float) || (data_type == ImGuiDataType_Double); is_floating_point)
        {
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        else if (g.ActiveIdSource == ImGuiInputSource_Keyboard || g.ActiveIdSource == ImGuiInputSource_Gamepad)
        {
            ...
            if (float input_delta = (axis == ImGuiAxis_X) ? GetNavTweakPressedAmount(axis) : -GetNavTweakPressedAmount(axis); input_delta != 0.0f)
            {
                const bool tweak_slow = IsKeyDown((g.NavInputSource == ImGuiInputSource_Gamepad) ? ImGuiKey_NavGamepadTweakSlow : ImGuiKey_NavKeyboardTweakSlow);
                const bool tweak_fast = IsKeyDown((g.NavInputSource == ImGuiInputSource_Gamepad) ? ImGuiKey_NavGamepadTweakFast : ImGuiKey_NavKeyboardTweakFast);
                const int decimal_precision = is_floating_point ? ImParseFormatPrecision(format, 3) : 0;
            ...
            else if (g.SliderCurrentAccumDirty)
            {
                ...
                if (float delta = g.SliderCurrentAccum; (clicked_t >= 1.0f && delta > 0.0f) || (clicked_t <= 0.0f && delta < 0.0f)) [[unlikely]] // This is to avoid applying the saturation when already past the limits
                {
                    set_new_value = false;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const bool is_resizable = (flags & ImGuiInputTextFlags_CallbackResize) != 0;
    if (is_resizable)
        IM_ASSERT(callback); // Must provide a callback if you set the ImGuiInputTextFlags_CallbackResize flag!
    ...
    const bool init_reload_from_user_buf = (state && state->ReloadUserBuf);
    ...
    if (is_readonly && state && (render_cursor || render_selection))
    {
        const char* buf_end = NULL;
    ...
    const bool is_displaying_hint = (hint && (buf_display_from_state ? state->TextA.Data : buf)[0] == 0);
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        else if (is_enter_pressed || is_gamepad_validate)
        {
            ...
            if (bool ctrl_enter_for_new_line = (flags & ImGuiInputTextFlags_CtrlEnterForNewLine) != 0; !is_multiline || is_gamepad_validate || (ctrl_enter_for_new_line && !io.KeyCtrl) || (!ctrl_enter_for_new_line && io.KeyCtrl))
            {
                validated = true;
    ...
    if (g.ActiveId == id)
    {
        ...
        if (!revert_edit || (validated && (flags & ImGuiInputTextFlags_EnterReturnsTrue) != 0))
        {
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor)
        {
            ...
            if (bool cursor_is_visible = (!g.IO.ConfigInputTextCursorBlink) || (state->CursorAnim <= 0.0f) || ImFmod(state->CursorAnim, 1.20f) <= 0.80f; cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    if (!BeginTooltipEx(ImGuiTooltipFlags_OverridePrevious, ImGuiWindowFlags_None))
        return;
    if (const char* text_end = text ? FindRenderedTextEnd(text, NULL) : text; text_end > text)
    {
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiID storage_id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    const bool span_all_columns = (flags & ImGuiTreeNodeFlags_SpanAllColumns) != 0 && (g.CurrentTable);
    ImRect frame_bb;
...
{
    if (!req || !(req->SelectRequest)) // Support NULL parameter so both calls can be done from same spot.
        return -1;
    int idx = -1;
...
void ImGui::EndBoxSelect(const ImRect& scope_rect, ImGuiMultiSelectFlags ms_flags)
{
    ...
    // Scroll
    if ((ms_flags & ImGuiMultiSelectFlags_ScopeWindow) && (ms_flags & ImGuiMultiSelectFlags_BoxSelectNoScroll) == 0)
    {
...
ImGuiMultiSelectIO* ImGui::BeginMultiSelect(ImGuiMultiSelectFlags flags, int selection_size, int items_count)
{
    ...
    static_assert(!offsetof(ImGuiMultiSelectTempData, IO)); // Clear() relies on that.
...
ImGuiMultiSelectIO* ImGui::EndMultiSelect()
{
    ...
    IM_ASSERT(g.CurrentMultiSelect && storage->Window == g.CurrentWindow);
...
void ImGui::MultiSelectItemHeader(ImGuiID id, bool* p_selected, ImGuiButtonFlags* p_button_flags)
{
    ...
    if (ms->IsFocused)
    {
        ...
        if (ms->IsKeyboardSetRange)
        {
            ...
            if (storage->RangeSrcItem == item_data || is_range_dst || ms->RangeSrcPassedBy != ms->RangeDstPassedBy)
            {
...
void ImGui::MultiSelectAddSetRange(ImGuiMultiSelectTempData* ms, bool selected, int range_dir, ImGuiSelectionUserData first_item, ImGuiSelectionUserData last_item)
{
    ...
    {
        if (ImGuiSelectionRequest* prev = &ms->IO.Requests.Data[ms->IO.Requests.Size - 1]; prev->Type == ImGuiSelectionRequestType_SetRange && prev->RangeLastItem == ms->LastSubmittedItem && prev->Selected == selected)
        {
...
void ImGuiSelectionBasicStorage::ApplyRequests(ImGuiMultiSelectIO* ms_io)
{
    ...
    IM_ASSERT(AdapterIndexToStorageId);
...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, const ImVec2& size_arg)
{
    ...
    int idx_hovered = -1;
    if (const int values_count_min = (plot_type == ImGuiPlotType_Lines) ? 2 : 1; values_count >= values_count_min)
    {
...
void ImGuiMenuColumns::CalcNextTotalWidth(bool update_offsets)
{
    ...
    for (auto i = 0; i < IM_ARRAYSIZE(Widths); i++)
    {
...
bool ImGui::BeginViewportSideBar(const char* name, ImGuiViewport* viewport_p, ImGuiDir dir, float axis_size, ImGuiWindowFlags window_flags)
{
    ...
    if (ImGuiWindow* bar_window = FindWindowByName(name); !bar_window || bar_window->BeginCount == 0)
    {
...
static int IMGUI_CDECL TabItemComparerBySection(const void* lhs, const void* rhs)
{
    ...
    if (const int a_section = TabItemGetSectionIdx(a), b_section = TabItemGetSectionIdx(b); a_section != b_section)
...
void    ImGui::EndTabBar()
{
    ...
    if (!tab_bar)
    {
        IM_ASSERT_USER_ERROR(tab_bar, "Mismatched BeginTabBar()/EndTabBar()!");
        return;
    }
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    if ((tab_bar->Flags & ImGuiTabBarFlags_TabListPopupButton) != 0)
    ...
    for (auto tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        if ((!most_recently_selected_tab || most_recently_selected_tab->LastFrameSelected < tab->LastFrameSelected) && !(tab->Flags & ImGuiTabItemFlags_Button))
    ...
    for (auto section_n = 0; section_n < 3; section_n++)
    ...
    if (width_excess >= 1.0f && ((tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyResizeDown) || !central_section_is_visible))
    {
        ...
        for (auto tab_n = shrink_data_offset; tab_n < shrink_data_offset + shrink_data_count; tab_n++)
        {
    ...
    for (auto section_n = 0; section_n < 3; section_n++)
    {
        ...
        for (auto tab_n = 0; tab_n < section->TabCount; tab_n++)
        {
...
{
    if (tab_id != 0)
        for (auto n = 0; n < tab_bar->Tabs.Size; n++)
...
void ImGui::TabBarQueueReorderFromMousePos(ImGuiTabBar* tab_bar, ImGuiTabItem* src_tab, ImVec2 mouse_pos)
{
    ...
    int dst_idx = src_idx;
    for (auto i = src_idx; i >= 0 && i < tab_bar->Tabs.Size; i += dir)
    {
...
bool ImGui::TabBarProcessReorder(ImGuiTabBar* tab_bar)
{
    ...
    if (!tab1 || (tab1->Flags & ImGuiTabItemFlags_NoReorder))
        return false;
...
{
    ImGuiContext& g = *GImGui;
    if (ImGuiWindow* window = g.CurrentWindow; window->SkipItems)
        return false;
...
void    ImGui::EndTabItem()
{
    ...
    if (ImGuiTabItem* tab = &tab_bar->Tabs[tab_bar->LastTabItemIdx]; !(tab->Flags & ImGuiTabItemFlags_NoPushId))
        PopID();
}
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    ImVec2 size = TabItemCalcSize(label, p_open || (flags & ImGuiTabItemFlags_UnsavedDocument));
    ...
    if (IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByPopup) && (IsMouseClicked(1) || IsMouseReleased(1)) && !is_tab_button)
        TabBarQueueFocus(tab_bar, tab);
...
{
    ImGuiContext& g = *GImGui;
    if (g.CurrentTabBar && !(g.CurrentTabBar->Flags & ImGuiTabBarFlags_DockNode))
    {
        ImGuiTabBar* tab_bar = g.CurrentTabBar;
