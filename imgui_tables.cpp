...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    if (table->IsInitializing)
    {
        ...
        for (auto n = 0; n < columns_count; n++)
        {
...
void ImGui::TableBeginApplyRequests(ImGuiTable* table)
{
    ...
    if (table->InstanceCurrent == 0)
    {
        ...
        if (table->ReorderColumn != -1 && table->ReorderColumnDir != 0)
        {
            ...
            for (auto order_n = src_order + reorder_dir; order_n != dst_order + reorder_dir; order_n += reorder_dir)
    ...
    {
        for (auto n = 0; n < table->ColumnsCount; n++)
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    [[assume(count_fixed >= 0 && count_stretch >= 0)]];
    int prev_visible_column_idx = -1;
    bool has_auto_fit_request = false;
    bool has_resizable = false;
    float stretch_sum_width_auto = 0.0f;
    float fixed_max_width_auto = 0.0f;
    for (auto order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
    ...
    if (width_remaining_for_stretched_columns >= 1.0f && !(table->Flags & ImGuiTableFlags_PreciseWidths))
        for (auto order_n = table->ColumnsCount - 1; stretch_sum_weights > 0.0f && width_remaining_for_stretched_columns >= 1.0f && order_n >= 0; order_n--)
        {
    ...
    if (!has_at_least_one_column_requesting_output)
    {
    ...
    if (!(table->DisableDefaultContextMenu) && TableBeginContextMenuPopup(table))
    {
...
void    ImGui::EndTable()
{
    ...
    IM_ASSERT(table && "Only call EndTable() if BeginTable() returns true!");
...
void ImGui::TableSetupColumn(const char* label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    IM_ASSERT(table && "Need to call TableSetupColumn() after BeginTable()!");
...
void ImGui::TableSetupScrollFreeze(int columns, int rows)
{
    ...
    for (auto column_n = 0; column_n < table->FreezeColumnsRequest; column_n++)
    {
...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    if (is_visible)
    {
        ...
        if (draw_cell_bg_color)
        {
            ...
            for (auto* cell_data = &table->RowCellData[0]; cell_data <= cell_data_end; cell_data++)
            {
...
void ImGui::TableSetColumnWidth(int column_n, float width)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    IM_ASSERT(table && !(table->IsLayoutLocked));
    IM_ASSERT(column_n >= 0 && column_n < table->ColumnsCount);
...
void ImGui::TableUpdateColumnsWeightFromWidth(ImGuiTable* table)
{
    ...
    [[assume(visible_weight >= 0.0f && visible_width >= 0.0f)]];
    for (auto column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ImGuiTableColumn* column = &table->Columns[column_n];
...
void ImGui::TableSetupDrawChannels(ImGuiTable* table)
{
    ...
    [[assume(freeze_row_multiplier > 0 && channels_for_row > 0 && channels_for_bg > 0 && channels_for_dummy >= 0 && channels_total > 0)]];
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    for (auto column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        [[assume(merge_group_sub_count > 0)]];
        for (auto merge_group_sub_n = 0; merge_group_sub_n < merge_group_sub_count; merge_group_sub_n++)
        {
            ...
            if (src_channel->_CmdBuffer.Size > 0 && src_channel->_CmdBuffer.back().ElemCount == 0 && !(src_channel->_CmdBuffer.back().UserCallback)) // Equivalent of PopUnusedDrawCmd()
...
#if 0
    if (g.IO.KeyShift)
        for (auto merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            MergeGroup* merge_group = &merge_groups[merge_group_n];
            if (merge_group->ChannelsCount == 0)
                continue;
    ...
    if (merge_group_mask != 0)
    {
        ...
        IM_ASSERT(!has_freeze_v || table->Bg2DrawChannelUnfrozen != TABLE_DRAW_CHANNEL_BG2_FROZEN);
        ...
        for (auto merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount)
            {
                ...
                for (auto n = 0; n < (size_for_masks_bitarrays_one >> 2); n++)
                    remaining_mask[n] &= ~merge_group->ChannelsMask[n];
                for (auto n = 0; n < splitter->_Count && merge_channels_count != 0; n++)
                {
        ...
        for (auto n = 0; n < splitter->_Count && remaining_count != 0; n++)
        {
...
IM_STATIC_ASSERT(!ImGuiSortDirection_None && ImGuiSortDirection_Ascending == 1 && ImGuiSortDirection_Descending == 2);
...
void ImGui::TableSetColumnSortDirection(int column_n, ImGuiSortDirection sort_direction, bool append_to_sort_specs)
{
    ...
    if (append_to_sort_specs)
        for (auto other_column_n = 0; other_column_n < table->ColumnsCount; other_column_n++)
...
void ImGui::TableHeadersRow()
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    IM_ASSERT(table && "Need to call TableHeadersRow() after BeginTable()!");
    ...
    for (auto column_n = 0; column_n < columns_count; column_n++)
    {
...
void ImGui::TableHeader(const char* label)
{
    ...
    ImGuiTable* table = g.CurrentTable;
    IM_ASSERT(table && "Need to call TableHeader() after BeginTable()!");
...
void ImGui::TableAngledHeadersRowEx(ImGuiID row_id, float angle, float max_label_width, const ImGuiTableHeaderData* data, int data_count)
{
    ...
    for (auto pass = 0; pass < 2; pass++)
        for (auto order_n = 0; order_n < data_count; order_n++)
        {
...
#if 0
    if ((flags_for_section_to_display & ImGuiTableFlags_Sortable) && column && (column->Flags & ImGuiTableColumnFlags_NoSort) == 0)
    {
        if (want_separator)
            Separator();
        want_separator = true;
...
static void TableSettingsInit(ImGuiTableSettings* settings, ImGuiID id, int columns_count, int columns_count_max)
{
    ...
    for (auto n = 0; n < columns_count_max; n++, settings_column++)
...
void ImGui::TableSaveSettings(ImGuiTable* table)
{
    ...
    for (auto n = 0; n < table->ColumnsCount; n++, column++, column_settings++)
    {
...
void ImGui::TableLoadSettings(ImGuiTable* table)
{
    ...
    for (auto data_n = 0; data_n < settings->ColumnsCount; data_n++, column_settings++)
    {
...
{
    ImGuiContext& g = *ctx;
    for (auto i = 0; i != g.Tables.GetMapSize(); i++)
...
static void TableSettingsHandler_WriteAll(ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
    ...
    for (auto* settings = g.SettingsTables.begin(); settings; settings = g.SettingsTables.next_chunk(settings))
    {
        ...
        for (auto column_n = 0; column_n < settings->ColumnsCount; column_n++, column++)
        {
            // "Column 0  UserID=0x42AD2D21 Width=100 Visible=1 Order=0 Sort=0v"
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    for (auto n = 0; n < table->InstanceCurrent + 1; n++)
    {
...
void ImGui::DebugNodeTableSettings(ImGuiTableSettings* settings)
{
    ...
    for (auto n = 0; n < settings->ColumnsCount; n++)
    {
...
ImGuiOldColumns* ImGui::FindOrCreateColumns(ImGuiWindow* window, ImGuiID id)
{
    ...
    for (auto n = 0; n < window->ColumnsStorage.Size; n++)
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags)
{
    ...
    IM_ASSERT(!(window->DC.CurrentColumns));   // Nested columns are currently not supported
    ...
    if (columns->Columns.Size == 0)
    {
        ...
        for (auto n = 0; n < columns_count + 1; n++)
        {
...
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems || !(window->DC.CurrentColumns))
        return;
...
void ImGui::EndColumns()
{
    ...
    if (!(flags & ImGuiOldColumnFlags_NoBorder) && !window->SkipItems)
    {
        ...
        for (auto n = 1; n < columns->Count; n++)
        {
            ImGuiOldColumnData* column = &columns->Columns[n];
        ...
        if (dragging_column != -1)
        {
            if (!columns->IsBeingResized)
                for (auto n = 0; n < columns->Count + 1; n++)
