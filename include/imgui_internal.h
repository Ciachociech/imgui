...
#define IMGUI_DEBUG_PRINTF(_FMT,...)    printf(_FMT __VA_OPT__(,) __VA_ARGS__)
#else
...
struct ImPool
{
    ...
    void        Clear()                             { for (auto n = 0; n < Map.Data.Size; n++) { int idx = Map.Data[n].val_i; if (idx != -1) Buf[idx].~T(); } Map.Clear(); Buf.clear(); FreeIdx = AliveCount = 0; }
...
struct ImGuiKeyRoutingTable
{
    ...
    void Clear()                    { for (auto n = 0; n < IM_ARRAYSIZE(Index); n++) Index[n] = -1; Entries.clear(); EntriesNext.clear(); }
};
...
#define IMGUI_TEST_ENGINE_LOG(_FMT,...)                     if (g.TestEngineHookItems) ImGuiTestEngineHook_Log(&g, _FMT __VA_OPT__(,) __VA_ARGS__)           // Custom log entry from user land into test log
#else
